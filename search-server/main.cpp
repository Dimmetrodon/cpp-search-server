...
const double EPSILON = 1e-6; //„исло дл€ сравнени€ double чисел
...
//—труктура id, релевантность, рейтинг
struct Document 
{
    int id;
    double relevance;
...
{
public:
    //ƒобавл€ет в stop_words_ стоп-слова
    ...
    //ƒобавл€ет в documents_ id, средний рейтинг, статус
    ...
    //—ортирует и возвращает 5 лучших по релевантности документа
...
private:
    //—труктура рейтинг, DocumentStatus(DocumentStatus::actuall,DocumentStatus::banned...)
    struct DocumentData 
    {
        int rating;
    ...
    //—ловарь слово - словарь(id,частота встречи в документах)
    map<string, map<int, double>> word_to_document_freqs_;
    //—ловарь id - DocumentData(рейтинг, статус)
    map<int, DocumentData> documents_;

    //явл€етс€ ли стоп-словом
    bool IsStopWord(const string& word) const 
    {
        return stop_words_.count(word) > 0;
    }

    //»з строки в вектор слов, исключа€ стоп-слова
    ...
    //—труктура слово - bool(минус-слово) - bool(плюс-слово)
    struct QueryWord 
    {
        string data;
        bool is_minus;
    ...
    //ѕринимает слово. ¬озвращает структуру слово - bool(минус-слово) - bool(плюс-слово)
    ...
    //—труктура множество плюс-слов - множество минус-слов
    struct Query 
    {
    ...
    //ƒелает из строки множества плюс и минус слов
